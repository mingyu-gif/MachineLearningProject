"""
LSTM 베이스라인 실험
기존 main_mlp.py 구조 그대로, 모델만 LSTM으로 변경
"""
...
def cross_validate(fd, window, lr, k_folds=3, epochs=20):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset_full)):
        ...
        model     = LSTM_Net(N_FEATURES, HIDDEN_DIM, NUM_LAYERS).to(DEVICE)
        ...
        result = train(
            ...
            device=DEVICE, output_dir="temp_lstm_cv/",
...
def train_final_model(fd, window, lr):
    ...
    result = train(
        ...
        device=DEVICE, output_dir=f"result_lstm_{fd}/",
        optimizer=optimizer, scheduler=scheduler,
        batch_size=BATCH_SIZE, epochs=EPOCHS,
        criterion=nn.MSELoss()
    )
